...
import { gio, homNay, ngayCoThu, tien } from '@/lib/format';
...
export default function HomNayPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <ViewBanner
        tieuDe={ngayCoThu(ngay)}
        phu={bep?.ten}
        badge={ngay === homNay() ? 'Hôm nay' : undefined}
      />
      ...
      {dangTai ? (
        ...
        <>
          ...
          <section className="flex flex-col gap-2">
            <h2 className="nhan px-1">Thực đơn {ngay === homNay() ? 'hôm nay' : 'ngày này'}</h2>
            <ThucDonHomNay ds={thucDon} ngay={ngay} />
          </section>

          <section className="flex flex-col gap-2">
            <h2 className="nhan px-1">Đơn gần nhất</h2>
            {don.length === 0 ? (
              <Trong
              ...
              <Card className="divide-y divide-border">
                {don.slice(0, 8).map((d) => (
                  <div key={d.id} className="flex items-center gap-3 p-3">
                    <span className="tabular w-12 shrink-0 text-sm font-semibold text-text-3">
                      {gio(d.created_at)}
                    </span>
                ...
                {don.length > 8 && (
                  <Link
                    href="/so-sach"
                    className="chu-mua flex items-center justify-center gap-1 p-3 text-sm font-bold"
                  >
