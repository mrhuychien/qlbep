        ...
        <div className="flex flex-col gap-1 rounded-xl border border-border bg-card p-1">
          {goiY.map((n) => (
            <button
              ...
              type="button"
              onClick={() => chon(n)}
              className="flex min-h-tap items-center justify-between gap-2 rounded-lg px-3 text-left transition-colors hover:bg-surface-2 active:bg-surface-2"
            >
          ...
          {!trungTen && (
            <button
              ...
              className="flex min-h-tap items-center gap-2 rounded-lg px-3 text-left font-semibold text-primary transition-colors hover:bg-surface-2 disabled:opacity-50"
            >
          ...
          {goiY.length === 0 && trungTen && (
            <p className="px-3 py-2 text-sm text-text-2">Đã có trong danh sách bên dưới.</p>
          )}
        </div>
      )}
    </div>
